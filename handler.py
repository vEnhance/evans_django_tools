...
class DiscordWebhookHandler(logging.Handler):
    ...
    def get_payload(self, record: logging.LogRecord) -> Payload:
        ...
        if hasattr(record, 'request'):
            ...
            chars_remaining = 1800 - sum(
                len(v) for v in description_parts.values())
            description_parts[
                ':blue_heart: REQUEST :blue_heart:'] = s[:chars_remaining]
    ...
    def get_url(self, record: logging.LogRecord) -> Optional[str]:
        return os.getenv(f'WEBHOOK_URL_{record.levelname.upper()}',
                         os.getenv('WEBHOOK_URL'))

    def get_response(self,
                     record: logging.LogRecord) -> Optional[requests.Response]:
